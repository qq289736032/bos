...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		if(rows.length != 1){
			...
			//发送ajax请求,请求定区action,在定区action中通过crm代理对象完成对应crm服务远程调用获取客户数据
			...
			//发送ajax请求,请求定区action,在定区action中通过crm代理对象完成对应crm服务远程调用获取客户数据decidedzoneId
			var url_1 = "decidedzoneAction_findListHasAssociation.action";
			var decidedzoneId = rows[0].id;
			$.post(url_1, {"id":decidedzoneId}, function(data){
				//alert(data);
				for(var i=0;i<data.length;i++){
					var id = data[i].id;
					var name = data[i].name
							...
							<script type="text/javascript">
								$(function(){
									...
									$("#associationBtn").click(function(){
										...
										//提交表单之前,需要将右侧下拉框中所有的选项选中,为option添加一个select属性
